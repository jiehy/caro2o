    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="服务单项备注" prop="serviceItemInfo">
          <el-input v-model="form.serviceItemInfo" placeholder="请输入服务单项备注" />
        </el-form-item>
        <el-form-item label="服务单项审核价格" prop="serviceItemPrice">
          <el-input v-model="form.serviceItemPrice" placeholder="请输入服务单项审核价格" />
        </el-form-item>
        ...
        <el-form-item label="状态【审核中0/审核拒绝1/审核通过2/审核撤销3】" prop="status">
          <el-select v-model="form.status" placeholder="请选择状态【审核中0/审核拒绝1/审核通过2/审核撤销3】">
            <el-option
              v-for="dict in dict.type.package_audit_status"
              :key="dict.value"
              :label="dict.label"
...
import { listPackageaudit, getPackageaudit, delPackageaudit, addPackageaudit, updatePackageaudit,
  listTodo,carPackageAudit,carPackageAuditHistory,carPackageAuditProcess,cancelCarPackageAudit
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPackageaudit(this.queryParams).then(response => {
        this.packageauditList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updatePackageaudit(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPackageaudit(this.form).then(response => {
